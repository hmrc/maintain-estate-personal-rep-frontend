...
import pages.business.{NamePage, TelephoneNumberPage}
...
class TelephoneNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  "TelephoneNumber controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val ua = baseAnswers.set(TelephoneNumberPage, validAnswer)
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val application =
        applicationBuilder(userAnswers = Some(baseAnswers.set(TelephoneNumberPage, validAnswer).success.value))
          .overrides(
